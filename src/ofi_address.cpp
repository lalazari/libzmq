...
int zmq::ofi_address_t::resolve_nic_name (const char *nic_, bool ipv6_, bool is_src_)
{
...
int zmq::ofi_address_t::resolve_interface (const char *interface_, bool ipv6_, bool is_src_)
{
...
int zmq::ofi_address_t::resolve_hostname (const char *hostname_, bool ipv6_, bool is_src_)
{
    //  Set up the query.
...
zmq::ofi_address_t::ofi_address_t () :
    _has_src_addr (false)
{
...
zmq::ofi_address_t::ofi_address_t (const sockaddr *sa, socklen_t sa_len) :
    _has_src_addr (false)
{
...
int zmq::ofi_address_t::resolve (const char *name_, bool local_, bool ipv6_, bool is_src_)
{
    if (!is_src_) {
...
int zmq::ofi_address_t::to_string (std::string &addr_)
{
    ...
    if (address.generic.sa_family == AF_INET6) {
        std::stringstream s;
        s << "ofi://[" << hbuf << "]:" << ntohs (address.ipv6.sin6_port);
        addr_ = s.str ();
    }
    else {
        std::stringstream s;
        s << "ofi://" << hbuf << ":" << ntohs (address.ipv4.sin_port);
        addr_ = s.str ();
    }
    return 0;
}

const sockaddr *zmq::ofi_address_t::addr () const
{
    return &address.generic;
}

socklen_t zmq::ofi_address_t::addrlen () const
{
    if (address.generic.sa_family == AF_INET6)
...
const sockaddr *zmq::ofi_address_t::src_addr () const
{
    return &source_address.generic;
}

socklen_t zmq::ofi_address_t::src_addrlen () const
{
    if (address.generic.sa_family == AF_INET6)
...
bool zmq::ofi_address_t::has_src_addr () const
{
    return _has_src_addr;
}

#if defined ZMQ_HAVE_WINDOWS
unsigned short zmq::ofi_address_t::family () const
#else
sa_family_t zmq::ofi_address_t::family () const
#endif
{
    return address.generic.sa_family;
}

zmq::ofi_address_mask_t::ofi_address_mask_t () :
    ofi_address_t (),
    address_mask (-1)
{
}

int zmq::ofi_address_mask_t::mask () const
{
    return address_mask;
}

int zmq::ofi_address_mask_t::resolve (const char *name_, bool ipv6_)
{
    ...
    const int rc =
        ofi_address_t::resolve_hostname (addr_str.c_str (), ipv6_);
    if (rc != 0)
        return rc;
...
int zmq::ofi_address_mask_t::to_string (std::string &addr_)
{
    if (address.generic.sa_family != AF_INET
    &&  address.generic.sa_family != AF_INET6) {
        addr_.clear ();
...
bool zmq::ofi_address_mask_t::match_address (const struct sockaddr *ss, const socklen_t ss_len) const
{
