...
void zmq::tune_ofi_socket (fd_t s_)
{
    ...
    int nodelack = 1;
    rc = setsockopt (s_, IPPROTO_TCP, OFI_NODELACK, (char*) &nodelack,
        sizeof (int));
    errno_assert (rc != SOCKET_ERROR);
#endif
}

void zmq::set_ofi_send_buffer (fd_t sockfd_, int bufsize_)
{
...
void zmq::set_ofi_receive_buffer (fd_t sockfd_, int bufsize_)
{
...
void zmq::tune_ofi_maxrt (fd_t sockfd_, int timeout_)
{
    if (timeout_ <= 0)
        return;

#if defined (ZMQ_HAVE_WINDOWS) && defined (OFI_MAXRT)
    // msdn says it's supported in >= Vista, >= Windows Server 2003
    timeout_ /= 1000;    // in seconds
    int rc = setsockopt (sockfd_, IPPROTO_TCP, OFI_MAXRT, (char*) &timeout_,
        sizeof(timeout_));
    wsa_assert (rc != SOCKET_ERROR);
#elif defined (OFI_USER_TIMEOUT)    // FIXME: should be ZMQ_HAVE_OFI_USER_TIMEOUT
    int rc = setsockopt (sockfd_, IPPROTO_TCP, OFI_USER_TIMEOUT, &timeout_,
        sizeof(timeout_));
    errno_assert (rc == 0);
#endif
}

 int zmq::ofi_write (fd_t s_, const void *data_, size_t size_)
{
#ifdef ZMQ_HAVE_WINDOWS
...
int zmq::ofi_read (fd_t s_, void *data_, size_t size_)
{
#ifdef ZMQ_HAVE_WINDOWS
