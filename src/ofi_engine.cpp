...
zmq::ofi_engine_t::ofi_engine_t (fd_t fd_, const options_t &options_,
    ...
    endpoint (endpoint_),
    plugged (false),
    next_msg (&ofi_engine_t::identity_msg),
    process_msg (&ofi_engine_t::process_identity_msg),
    io_error (false),
...
void zmq::ofi_engine_t::plug (io_thread_t *io_thread_,
    session_base_t *session_)
{
    ...
    if (options.raw_socket) {
        ...
        next_msg = &ofi_engine_t::pull_msg_from_session;
        process_msg = &ofi_engine_t::push_raw_msg_to_session;
    ...
    set_pollin (handle);
    set_pollout (handle);
    //  Flush all the data that may have been already received downofi.
    in_event ();
}

void zmq::ofi_engine_t::unplug ()
{
    zmq_assert (plugged);
    plugged = false;
...
void zmq::ofi_engine_t::terminate ()
{
    unplug ();
    delete this;
}

void zmq::ofi_engine_t::in_event ()
{
    zmq_assert (!io_error);
    ...
    if (!insize) {
        ...
        //  the underlying OFI layer has fixed buffer size and thus the
        //  number of bytes read will be always limited.
        size_t bufsize = 0;
        decoder->get_buffer (&inpos, &bufsize);

        const int rc = ofi_read (s, inpos, bufsize);
...
void zmq::ofi_engine_t::out_event ()
{
    zmq_assert (!io_error);
    ...
    //  arbitrarily large. However, we assume that underlying OFI layer has
    //  limited transmission buffer and thus the actual number of bytes
    //  written should be reasonably modest.
    const int nbytes = ofi_write (s, outpos, outsize);
...
void zmq::ofi_engine_t::restart_output ()
{
...
void zmq::ofi_engine_t::restart_input ()
{
...
bool zmq::ofi_engine_t::handshake ()
{
    zmq_assert (handshaking);
    zmq_assert (greeting_bytes_read < greeting_size);
    //  Receive the greeting.
    while (greeting_bytes_read < greeting_size) {
        const int n = ofi_read (s, greeting_recv + greeting_bytes_read,
    ...
    if (greeting_recv [0] != 0xff || !(greeting_recv [9] & 0x01)) {
        ...
        //  message into the incoming message ofi.
    ...
    if (greeting_recv [revision_pos] == ZMTP_2_0) {
        ...
        next_msg = &ofi_engine_t::next_handshake_command;
        process_msg = &ofi_engine_t::process_handshake_command;
    }
...
int zmq::ofi_engine_t::identity_msg (msg_t *msg_)
{
...
int zmq::ofi_engine_t::process_identity_msg (msg_t *msg_)
{
    ...
    if (subscription_required)
        process_msg = &ofi_engine_t::write_subscription_msg;
    else
        process_msg = &ofi_engine_t::push_msg_to_session;

    return 0;
}

int zmq::ofi_engine_t::next_handshake_command (msg_t *msg_)
{
...
int zmq::ofi_engine_t::process_handshake_command (msg_t *msg_)
{
    zmq_assert (mechanism != NULL);
...
void zmq::ofi_engine_t::zap_msg_available ()
{
    zmq_assert (mechanism != NULL);
...
void zmq::ofi_engine_t::mechanism_ready ()
{
    if (options.heartbeat_interval > 0) {
    ...
    next_msg = &ofi_engine_t::pull_and_encode;
    process_msg = &ofi_engine_t::write_credential;
...
int zmq::ofi_engine_t::pull_msg_from_session (msg_t *msg_)
{
    return session->pull_msg (msg_);
}

int zmq::ofi_engine_t::push_msg_to_session (msg_t *msg_)
{
    return session->push_msg (msg_);
}

int zmq::ofi_engine_t::push_raw_msg_to_session (msg_t *msg_) {
    if (metadata && metadata != msg_->metadata())
        msg_->set_metadata(metadata);
    return push_msg_to_session(msg_);
}

int zmq::ofi_engine_t::write_credential (msg_t *msg_)
{
    zmq_assert (mechanism != NULL);
    zmq_assert (session != NULL);
    ...
    process_msg = &ofi_engine_t::decode_and_push;
    return decode_and_push (msg_);
}

int zmq::ofi_engine_t::pull_and_encode (msg_t *msg_)
{
    zmq_assert (mechanism != NULL);
...
int zmq::ofi_engine_t::decode_and_push (msg_t *msg_)
{
    zmq_assert (mechanism != NULL);
    ...
    if (session->push_msg (msg_) == -1) {
        if (errno == EAGAIN)
            process_msg = &ofi_engine_t::push_one_then_decode_and_push;
        return -1;
    }
    return 0;
}

int zmq::ofi_engine_t::push_one_then_decode_and_push (msg_t *msg_)
{
...
int zmq::ofi_engine_t::write_subscription_msg (msg_t *msg_)
{
    msg_t subscription;
...
void zmq::ofi_engine_t::error (error_reason_t reason)
{
...
void zmq::ofi_engine_t::set_handshake_timer ()
{
...
bool zmq::ofi_engine_t::init_properties (properties_t & properties) {
    if (peer_address.empty()) return false;
    properties.insert (std::make_pair("Peer-Address", peer_address));
    return true;
}

void zmq::ofi_engine_t::timer_event (int id_)
{
    if(id_ == handshake_timer_id) {
        ...
        next_msg = &ofi_engine_t::produce_ping_message;
        out_event();
        add_timer(options.heartbeat_interval, heartbeat_ivl_timer_id);
    }
...
int zmq::ofi_engine_t::produce_ping_message(msg_t * msg_)
{
    int rc = 0;
    zmq_assert (mechanism != NULL);
...
int zmq::ofi_engine_t::produce_pong_message(msg_t * msg_)
{
    int rc = 0;
    zmq_assert (mechanism != NULL);
...
int zmq::ofi_engine_t::process_heartbeat_message(msg_t * msg_)
{
    if(memcmp(msg_->data(), "\4PING", 5) == 0) {
        ...
        next_msg = &ofi_engine_t::produce_pong_message;
        out_event();
    }
